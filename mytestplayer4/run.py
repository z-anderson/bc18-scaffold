        ...
        for unit in gc.my_units():
            for unit in gc.my_units():
                ...
                for rocket in rocketFinished:  # build
                    if gc.can_load(unit.id, rocket):
                        gc.load(unit.id, rocket)
                        break
                if unit.location.is_in_garrison(): continue
            ...
            if unit.unit_type == bc.UnitType.Worker:
                ...
                if numRockets < 3 and gc.karbonite() > bc.UnitType.Rocket.blueprint_cost():
                    for d in directions:
                        if gc.can_blueprint(unit.id, bc.UnitType.Rocket, d):
                            gc.blueprint(unit.id, bc.UnitType.Rocket, d)
                            workerJob["r"] += 1
                            print("created r blueprint")
                            workerUsed = True
                            break
                    if workerUsed: continue
            ...
            if unit.unit_type == bc.UnitType.Factory:
                ...
                if len(garrison) > 0:#ungarrison
                    ...
                    if gc.can_unload(unit.id, d):
                        ...
                        continue
                if gc.can_produce_robot(unit.id, bc.UnitType.Ranger):#produce Mages
